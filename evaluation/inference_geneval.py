...
def main():
    ...
    skip_grid  = config.inference.get('skip_grid', True)
    ...
    output_path_name = config.training.get('img_log_path',  f"t2i_samples_step{config.training.generation_timesteps}_scale{config.training.guidance_scale}")
    ...
    for batch_index in trange(0, metadata_len, num_processes, desc="Generating images"):
        ...
        with distributed_state.split_between_processes(batch_raw) as batch:
            ...
            if batch is not None and index < metadata_len:
                ...
                prompt = [batch[0]['prompt']] * batch_size
                print(f"Prompt ({index: >3}/{len(metadatas)}): '{prompt[0]}'")
                ...
                if not skip_grid:
                    all_samples.append(images.clone())
                images *= 255.0
