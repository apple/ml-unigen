...
#
# Started from https://github.com/showlab/Show-o/blob/main/training/imagenet_dataset.py
...
class ImageNetDataset(DatasetFolder):
    def __init__(
        ...
        super().__init__(
            os.path.join(data_dir, root) if data_dir else root,
            loader,
        ...
        with open('data/imagenet1k_label_mapping', 'r') as f:
            self.labels = {}
            for l in f:
    ...
    def __getitem__(self, idx):
        try:
            ...
            root, _ = os.path.splitext(os.path.split(path)[-1])
            image = self.loader(path)
            image = self.transform(image, resolution=self.image_size)
            _, synset_id = os.path.basename(root).rsplit("_", 1)
            input_ids = IMAGENET2012_CLASSES[synset_id]
            input_ids = random.choice(input_ids.split(',')).strip()
            if self.prefix_prompt is not None:
                input_ids = self.prefix_prompt  + input_ids
